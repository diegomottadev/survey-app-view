...
useEffect(() => {
  async function loadSizes() {
    try {
      const { data: { data: result } } = await SizesServices.all(pet)
      setSizes(result);
    } catch (err) {
      console.log(err);
...
useEffect(() => {
  async function loadNecessities() {
    try {
      const { data: { data: result } } = await NecessitiesService.all(pet)
      setNecessities(result);
    } catch (err) {
      console.log(err);
      console.warn('Hubo un problema con la carga del listado de necesidades');
    }
  }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        ...
        navigate('/product',   { replace: true, state: {surveyCreated: response,product:producRecommended} } );
